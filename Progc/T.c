...
void traiter(VilleAVL * root, FILE * dataFile){
    if (root != NULL){
        traiter(root->gauche,dataFile);
        fprintf(dataFile, "%s;%d;%d\n", root->ville, root->nb_passage_ville, root->nb_passage_ville_depart);
        traiter(root->droite,dataFile);
    }
}
